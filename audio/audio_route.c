...
static int path_apply(struct audio_route *ar, struct mixer_path *path)
{
    ...
    for (i = 0; i < path->length; i++) {
	    ...
	    ALOGW("audio_route_init mixer_state[%d].new_value[0]=%d", i, ar->mixer_state[ctl_index].new_value[0]);
...
void update_mixer_state(struct audio_route *ar)
{
    ...
    unsigned int j;
    int ret;
    ALOGW("update_mixer_state, total=%d", ar->num_mixer_ctls);
    ...
    for (i = 0; i < ar->num_mixer_ctls; i++) {
        ...
        if (ar->mixer_state[i].new_linked) {
		    ...
		    ALOGW("mixer_state[%d].new_value[0]=%d", i, ar->mixer_state[i].new_value[0]);
                }
...
struct audio_route *audio_route_init(void)
{
    ...
    ALOGW("audio_route_init, total ctls=%d", ar->num_mixer_ctls);
    for (i=0; i<ar->num_mixer_ctls; i++) {
	ALOGW("mixer_ctl[%d] = %s", i, mixer_ctl_get_name(ar->mixer_state[i].ctl));
    }
